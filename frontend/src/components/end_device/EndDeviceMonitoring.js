...
function EndDeviceMonitoring() {
    ...
    useEffect(() => {
        fetch(`/api/end_device?devEui=${devEui}`)
            .then(response => response.json())
            .then(data => setEndDevice(data.data))
    }, [])
    return (<Container sx={{minWidth: 1500, margin: "auto", overflow: "hidden"}}>
        ...
        <Typography sx={{mb: 1}} color="text.dark" variant="body2">
            Dev EUI: {endDevice.devEui}
        </Typography><Typography sx={{mb: 1}} color="text.dark" variant="body2">
        App EUI: {endDevice.appEui}
    </Typography>
        ...
        {page === 0 && <Overview endDeviceInfo={endDevice}/>}
        {page === 1 && Object.keys(endDevice).length &&
            <LiveData endDeviceInfo={endDevice}/>}
...
export default EndDeviceMonitoring;
